...
class Moveit(object):
    ...
    # For this to work we need to unpause gazebo
...
if __name__ == '__main__':
    ...
    # print("Joint values: ", move_ur5e_object.get_joint_angles_robot_arm())
    ...
    # print("Gripper Joint names: ", move_ur5e_object.gripper.get_joints())
    # print("Gripper Joint values: ", move_ur5e_object.get_joint_angles_gripper())
    ...
    # Get the reference frame from the PlanningSceneInterface
    reference_frame = move_ur5e_object.robot.get_planning_frame()

    # Print the reference frame
    print("MoveIt reference frame: {}".format(reference_frame))
    ...
    ros_gazebo.gazebo_unpause_physics()
    header, pose, twist, success = ros_gazebo.gazebo_get_model_state(model_name=model_name,
                                                                     relative_entity_name="rx200/base_link")
    ros_gazebo.gazebo_pause_physics()

    print("pose of the cube with ref to 'rx200/base_link':", pose.position)
    rpy = euler_from_quaternion([pose.orientation.x, pose.orientation.y, pose.orientation.z, pose.orientation.w])
    print("rpy of the cube with ref to 'rx200/base_link':", list(rpy))
    print("type:", type(list(rpy)))

    ros_gazebo.gazebo_unpause_physics()
    header1, pose1, twist1, success1 = ros_gazebo.gazebo_get_model_state(model_name=model_name,
                                                                         relative_entity_name="world")
    ros_gazebo.gazebo_pause_physics()

    print("pose of the cube with ref to 'world':", pose1.position)
    print("rpy of the cube with ref to 'world':",
          euler_from_quaternion([pose1.orientation.x, pose1.orientation.y, pose1.orientation.z, pose1.orientation.w]))

    print("offset in z:", pose1.position.z - pose.position.z)
